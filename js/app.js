...
function renderizarFichaEquipo(equipoId) {
    ...
    dbPartidos.forEach(p => {
        ...
        // Asistencias
        p.eventos.asistencias.forEach(a => {
            if (a.equipo === equipoId) {
                if (!estadisticasJugadoras[a.jugadora]) estadisticasJugadoras[a.jugadora] = { goles: 0, asistencias: 0 };
                estadisticasJugadoras[a.jugadora].asistencias++;
            }
        });
    });
        ...
        jugadorasLista.forEach(nom => {
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>${estadisticasJugadoras[nom].asistencias}</td>
                </tr>`;
        });
    }
